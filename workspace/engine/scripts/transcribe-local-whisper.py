...
def load_whisper_model(model_name: str, device: str, model_cache_dir: Path):
    try:
        ...
        sys.exit(
            ...
            "Install the repo-local runtime with: ./workspace/engine/scripts/setup-local-transcription.sh"
        )

    if not hasattr(whisper, "load_model"):
        sys.exit(
            "Imported a Python package named 'whisper', but it is not OpenAI Whisper.\n"
            "Install the expected repo-local runtime with: ./workspace/engine/scripts/setup-local-transcription.sh"
        )
