...
under workspace/references/<slug>/ so Codex can inspect frames, read transcript
segments, and fill workspace/engine/templates/reference-analysis.md without a hosted app.
"""
...
DEFAULT_OUT_ROOT = REPO_ROOT / "workspace" / "references"
...
def local_whisper_segments(video_path: Path, out_dir: Path, language: str | None) -> list[dict]:
    transcriber = REPO_ROOT / "workspace" / "engine" / "scripts" / "transcribe-local-whisper.py"
    python_bin = REPO_ROOT / ".venv" / "bin" / "python"
    if not python_bin.exists():
        sys.exit(
            "Local Whisper requested, but .venv/bin/python does not exist. "
            "Run ./workspace/engine/scripts/setup-local-transcription.sh first."
        )
...
def write_analysis_stub(out_dir: Path, source: str, info: dict, platform: str, duration: float) -> None:
    template_path = REPO_ROOT / "workspace" / "engine" / "templates" / "reference-analysis.md"
...
After analysis, update `workspace/references/REFERENCES.md` with one row. Promote only
concise reusable lessons to `workspace/channel/STYLE_GUIDE.md`; do not copy one-off style
choices blindly.
"""
