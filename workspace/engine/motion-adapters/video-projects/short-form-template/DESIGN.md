# Design

Use the repo root `workspace/channel/DESIGN.md` as the channel style source.

This project is a 9:16 short-form starter for agentic workflow tutorials and short explainers.

## Format

- Width: 1080
- Height: 1920
- Duration: 12 seconds
- Safe content area: keep captions and face framing away from the very bottom UI zone.

## Edit Notes

- Replace `.face-temporary` with A-roll, an ACS render, or a product capture.
- Keep captions large enough to read on a phone.
- Motion should land on the hook, keyword, or proof moment.
