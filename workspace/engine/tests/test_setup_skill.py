...
class SetupSkillTests(unittest.TestCase):
    def setUp(self) -> None:
        self.root = REPO_ROOT / "workspace" / "productions" / "test-runs" / "setup-skill"
        if self.root.exists():
    ...
    def test_profile_copy_is_exact_and_routes_follow_enabled_policy(self) -> None:
        profile = REPO_ROOT / "workspace" / "channel" / "brand.json"
    ...
    def test_disabled_delivery_default_is_rejected_before_workspace_write(self) -> None:
        profile = read_json(REPO_ROOT / "workspace" / "channel" / "brand.json")
    ...
    def test_setup_skill_is_concise_and_metadata_is_invocable(self) -> None:
        ...
        self.assertIn("validate-profile workspace/channel/brand.json", skill)
